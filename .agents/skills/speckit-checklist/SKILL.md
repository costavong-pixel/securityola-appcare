...
**Check for extension hooks (report only; never execute automatically) (before checklist generation)**:
...
- For each executable hook, output the following based on its `optional` flag:
  ...
  - **Mandatory hook** (`optional: false`):
    ...
    **Automatic Pre-Hook**: {extension}
    ...
    Do not run the hook; report it to Codex for explicit approval before proceeding to the Execution Steps.
    ```
...
**Check for extension hooks (report only; never execute automatically) (after checklist generation)**:
Check if `.specify/extensions.yml` exists in the project root.
